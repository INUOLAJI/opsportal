...
import {
  ...
  Menu, Search, Moon, Sun, Loader2, Lock
} from 'lucide-react';
...
function SettingsPage() {
  ...
  const handleDiscard = () => {
    if (savedSettings) setForm(mapSettingsToForm(savedSettings));
    setSaveError(null);
  };
    ...
    >
      ...
      >
        ...
        <main className="p-3 p-md-4 flex-grow-1" style={{ paddingTop: 'calc(64px + 1.5rem)' }}>
          ...
          {!isAdmin && !loading && (
            <div className="alert py-2 px-3 small mb-3 d-flex align-items-center gap-2" style={{ backgroundColor: darkMode ? 'rgba(59, 130, 246, 0.1)' : '#EFF6FF', color: '#3B82F6', border: 'none' }}>
              <Lock size={14} />
              You're viewing these settings in read-only mode — only administrators can make changes.
            </div>
          )}
          ...
          {loadError && (
            <div className="alert alert-danger py-2 px-3 small mb-3 d-flex justify-content-between align-items-center">
              {loadError}
              <button className="btn btn-sm btn-link p-0 text-decoration-underline" onClick={loadSettings}>Retry</button>
            </div>
          )}
              ...
              <div className="card border-0 p-4 rounded-3" style={{ backgroundColor: cardBg, boxShadow: '0 1px 3px rgba(0, 0, 0, 0.05)' }}>
                ...
                {loading || !form ? (
                  <div className="text-center py-5" style={{ color: textSecondary }}>
                    ...
                    <div className="small mt-2">Loading settings…</div>
                  </div>
                ) : (
                  <>
                    ...
                    {activeTab === 'security' && (
                      <div>
                        ...
                        {rotateError && <div className="alert alert-danger py-2 px-3 small mb-3">{rotateError}</div>}
                    ...
                    {/* FOOTER ACTION PANEL */}
                    {isAdmin && (
                      <div className="mt-5 pt-3 border-top d-flex justify-content-end align-items-center gap-2" style={{ borderColor: borderColor }}>
                        {saveSuccess && !hasChanges && (
                          <span className="small me-auto" style={{ color: '#10B981' }}>
